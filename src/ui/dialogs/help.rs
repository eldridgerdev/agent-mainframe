...
fn draw_help_at(frame: &mut Frame, area: Rect, scroll_offset: usize, theme: &Theme) {
    ...
    let pr_review_keybinds: Vec<(&str, &str)> = vec![
        ...
        ("f", "Fix this comment — or, if routed to investigate,"),
        ("", "run a read-only investigation of it (pick a harness)"),
        ...
        ("v", "Toggle this comment: fix ⇄ investigate (read-only)"),
        ("a", "Act on a finished investigation: convert to fix,"),
        ("", "add to batch, post reply, ask follow-up, dismiss,"),
        ("", "keep as TODO"),
