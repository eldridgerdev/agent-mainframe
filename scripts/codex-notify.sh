...
# Codex's notify hook fires once, when the turn ends, and carries nothing that
# separates "I'm asking you something" from "I'm done". Reporting "completed"
# is the strongest claim the signal supports; AMF narrows anything it can't
# justify to a generic Waiting.
ATTENTION_MSG="$(jq -nc \
    --arg sid "$SESSION_ID" \
    --arg cwd "$CWD" \
    --arg provider_session_id "$PROVIDER_SESSION_ID" \
...
if command -v "$AMF_CMD" >/dev/null 2>&1; then
    ...
    echo "$ATTENTION_MSG" | "$AMF_CMD" notify 2>/dev/null || true
fi
