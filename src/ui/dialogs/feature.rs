...
fn draw_create_feature_session_name(frame: &mut Frame, state: &CreateFeatureState, theme: &Theme) {
    let area = centered_rect(50, 25, frame.area());
    crate::ui::draw_modal_overlay(frame, area, theme);

    let title = format!(" Name Session ({}) ", state.project_name);
...
pub fn draw_steering_prompt_dialog(
    ...
    let prompt_text = super::editor_view::editor_lines(
        &state.editor,
        theme,
        "Describe the task, then add boundaries, validation, and watch-outs.",
    );
